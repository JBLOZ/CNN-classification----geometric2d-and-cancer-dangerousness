...
%
% Estructura del dataset (data.zip):
...
%
% Instrucciones:
%   1. Coloca "data.zip" en el directorio actual de MATLAB.
%   2. Si existe "trainedModel.mat", se carga el modelo; de lo contrario, se
%      extrae data.zip, se entrena el modelo usando los datos de "train" y "val",
%      y se guarda en "trainedModel.mat".
...
if isfile('trainedModel.mat')
    ...
    fprintf('Modelo entrenado encontrado y cargado desde trainedModel.mat\n');
else
    % Si no existe, extraer el dataset y entrenar el modelo
    if exist('data.zip', 'file')
        fprintf('Extrayendo data.zip...\n');
        unzip('data.zip');  % Extrae el contenido del ZIP en el directorio actual
    else
        error('No se encontró data.zip. Colócalo en el directorio actual.');
    end
    ...
    fprintf('Modelo entrenado guardado en trainedModel.mat\n');
end
